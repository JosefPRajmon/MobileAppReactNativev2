...
import { updateModules, addReadedItemId } from '../store/actions/ModulesActions';
...
export default function OnSiteListScreen({ navigation }: any) {
    const dispatch = useDispatch();
    const db: any = DatabaseProvider.getInstance();
    const readedItemsHidden = useSelector((state: any) => state.modules[OnSiteConfig.moduleID]?.readedItemsHidden);
    const readedItems = useSelector((state: any) => state.modules[OnSiteConfig.moduleID]?.readedItems || [])
    ...
    const getData = async () => {
        db.loadData("SELECT * FROM on_site ORDER BY pub_date DESC")
            .then((data: any) => {
    ...
    const onRefresh = async () => {
        ...
        await dispatch(updateModules(OnSiteConfig.moduleID));
        await getData();
        Toast.show({
    ...
    const handleItemPress = async (item: any) => {
        item.isReaded || await dispatch(await addReadedItemId(OnSiteConfig.moduleID, item.id_news));
